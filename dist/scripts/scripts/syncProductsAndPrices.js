...
async function syncProductsAndPrices() {
    const products = await stripe_1.stripe.products.list({ active: true });
    for (const product of products.data) {
        const prices = await stripe_1.stripe.prices.list({ product: product.id });
            ...
            for (const price of prices.data) {
                await tx.price.upsert({
                    ...
                    update: {
                        ...
                        recurring_usage_type: price.recurring?.usage_type ?? "licensed",
                        active: price.active,
                    },
                    create: {
                        price_id: price.id,
        ...
        console.log(`Synced product ${product.id} with ${prices.data.length} price(s)`);
    }
