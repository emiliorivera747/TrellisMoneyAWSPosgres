...
const HeaderTitle = ({ year }: { year: number }) => (
  <>
    <span className="text-sm tracking-wider pl-4">Financial Projections</span>
...
const HeaderDropdown = ({
  ...
  <DropdownMenu>
    <DropdownMenuTrigger asChild>
      <button
        onClick={() => setMode()}
        className="flex justify-end pr-6 text-tertiary-700 hover:text-tertiary-800"
      >
        {CreateSvgV3({
          paths: [
            {
    ...
    <DropdownMenuContent align="end">
      ...
      <DropdownMenuSeparator />
      {mode === "view" && <DropdownMenuItem onClick={() => setMode()}>Edit</DropdownMenuItem>}
      {mode === "edit" && <DropdownMenuItem onClick={() => setMode()}>View</DropdownMenuItem>}
    </DropdownMenuContent>
  </DropdownMenu>
);
